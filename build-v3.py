...
<title>Simply Straight — El secreto de las bien peinadas</title>
...
<div class="urgency-bar" id="urgencyBar">
    🔥 SOLO <span id="stockCounter">67</span> UNIDADES DISPONIBLES · ENVÍO GRATIS HOY
</div>
...
<section class="hero">
    <div class="hero-content">
        ...
        <h1>El secreto de las que <span class="rose italic">siempre andan bien peinadas.</span></h1>
        <p class="hero-sub">Cabello listo en 60 segundos. Sin plancha, sin cremas, sin quemarlo.</p>
        
        <!-- PRECIO EN HERO -->
        <div style="text-align:center;margin:16px 0;">
            <span style="font-size:13px;color:var(--gray-400);text-decoration:line-through;margin-right:8px;">$187.500</span>
            <span style="font-size:28px;font-weight:800;color:var(--charcoal);">$97.500</span>
        ...
        <div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap;margin-top:4px;">
            <span style="background:var(--gray-100);padding:4px 10px;border-radius:20px;font-size:11px;font-weight:600;">1 × $97.500</span>
            <span style="background:var(--rose-light);color:var(--rose-dark);padding:4px 10px;border-radius:20px;font-size:11px;font-weight:700;">2 × $87.500 c/u</span>
            <span style="background:var(--gold);color:#7c6800;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:700;">3 × $77.500 c/u</span>
        </div>
        <!-- FIN PRECIO HERO -->
    </div>
    ...
    <div style="width:100%;max-width:420px;height:340px;margin:0 auto 16px;padding:0 20px;">
        <img src="creativos/hero-producto-doble.png" alt="Simply Straight — El secreto de las bien peinadas" style="width:100%;height:100%;object-fit:cover;border-radius:20px;box-shadow:0 12px 40px rgba(0,0,0,.12);">
    </div>
...
<section class="section" style="padding-top:0;">
    <div class="container">
        <video class="hero-video" autoplay loop muted playsinline poster="creativos/antes y despues/1.png" onclick="this.play();this.muted=false" style="max-width:360px;margin:0 auto;display:block;" controlsList="nodownload" oncontextmenu="return false" disablepictureinpicture>
...
<section class="section" style="padding-top:0;">
    <div class="container" style="text-align:center;">
        ...
        <a href="https://wa.me/573186606070" class="btn-block btn-outline">💬 Preguntar por WhatsApp</a>
    </div>
</section>
        ...
        <div class="checkout-card">
            ...
            <div style="margin:20px 0;">
                    ...
                    <div class="payment-tabs" id="qtyTabs" style="margin-bottom:16px;">
                        <div class="payment-tab active" onclick="selectQty(this,1,97500)" style="flex-direction:column;">
                            <span style="font-weight:800;font-size:15px;">1 unidad</span>
                            <span style="font-size:11px;">$97.500</span>
                        </div>
                        <div class="payment-tab" onclick="selectQty(this,2,87500)" style="flex-direction:column;border-color:var(--rose-light);">
                            <span style="font-weight:800;font-size:15px;">2 unidades</span>
                            <span style="font-size:11px;">$87.500 c/u</span>
                            <span style="font-size:9px;color:var(--green);">AHORRAS $40.000</span>
                        </div>
                        <div class="payment-tab" onclick="selectQty(this,3,77500)" style="flex-direction:column;border-color:var(--gold);">
                            <span style="font-weight:800;font-size:15px;">3 unidades</span>
                            <span style="font-size:11px;">$77.500 c/u</span>
            ...
            <form class="checkout-form" id="orderForm">
                ...
                <button type="submit" class="submit-btn">✅ CONFIRMAR PEDIDO — $97.500</button>
            ...
            <div style="text-align:center;margin-top:20px;">
                ...
                <a href="https://wa.me/573186606070?text=Hola!%20Quiero%20comprar%20Simply%20Straight" class="btn-block btn-rose">💬 Comprar directamente por WhatsApp</a>
            </div>
        </div>
    </div>
</section>
...
<!-- FLOATING WHATSAPP -->
<a href="https://wa.me/573186606070" class="float-wa" target="_blank" title="Escríbenos por WhatsApp">
...
function showToast() {
    if (stock <= 5) return; // stop at 5 units
    var city = cities[Math.floor(Math.random() * cities.length)];
    var name = firstNames[Math.floor(Math.random() * firstNames.length)] + ' ' + lastNames[Math.floor(Math.random() * lastNames.length)];
    var q = Math.random() > 0.6 ? 2 : 1; // mostly 1 unit, occasionally 2
    var actualDrop = Math.min(q, stock - 5); // never go below 5
    if (actualDrop <= 0) return;
    stock -= actualDrop;
    document.getElementById('stockCounter').textContent = stock;
    if (stock <= 8) { document.getElementById('urgencyBar').style.background = '#dc2626'; }
...
document.getElementById('orderForm').addEventListener('submit', function(e) {
    ...
    var payEl = document.querySelector('#paymentTabs .payment-tab.active');
    var pay = payEl ? payEl.textContent.replace(/\\n|📦|💳|🏦|📱/g,'').trim() : 'Contraentrega';
    ...
    msg += '%0A📅 ' + order.fecha;
    msg += '%0A👤 *Cliente:* ' + order.nombre + ' ' + order.apellido;
    msg += '%0A📱 *Celular:* ' + order.celular;
    msg += '%0A📧 *Correo:* ' + order.correo;
    msg += '%0A📍 *Ubicación:* ' + order.ciudad + ', ' + order.departamento;
    msg += '%0A🏠 *Dirección:* ' + order.direccion;
    msg += '%0A🏘️ *Barrio:* ' + order.barrio;
    msg += '%0A📦 *Producto:* Simply Straight x' + order.cantidad;
    msg += '%0A💰 *Precio u:* $' + order.precioUnit.toLocaleString('es-CO');
    msg += '%0A💵 *TOTAL:* $' + order.total.toLocaleString('es-CO');
    msg += '%0A💳 *Pago:* ' + order.pago;
    
    // Open MercadoPago or WhatsApp
    var waURL = 'https://wa.me/573186606070?text=' + msg;
    if (pay === 'MercadoPago') {
